...
//
// File: classes/BadPlanetReportCE.php
//
...
class BadPlanetReportCE
{
    public static function buildBase($db, $langvars, $planet_id, $sector_id)
    {
        ...
        if (!is_numeric ($planet_id) || !is_numeric ($sector_id))
        {
            ...
            $hack_id = 0x1337;
            BntAdminLog::writeLog ($db, LOG_ADMIN_PLANETCHEAT, "{$hack_id}|{$ip}|{$planet_id}|{$sector_id}|{$playerinfo['ship_id']}");
            echo "<div style='color:#f00; font-size:16px;'>" . $langvars['l_pr_make_base_failed'] . "</div>\n";
    ...
    public static function collectCredits($db, $langvars, $planetarray, $sector_max)
    {
        ...
        BntDb::logDbErrors ($db, $result1, __LINE__, __FILE__);
        ...
        for ($i = 0; $i < $temp_count2 && $CS == "GO"; $i++)
        {
            ...
            elseif ($CS == "GO")
            {
                $CS = BadPlanetReportCE::takeCredits ($db, $langvars, $s_p_pair[$i][0], $s_p_pair[$i][1]);
            }
            else
            {
    ...
    //  This should patch the game from being hacked with planet Hack.
        ...
        if ($planet_hack)
        {
            ...
            BntAdminLog::writeLog ($db, LOG_ADMIN_PLANETCHEAT+1000, "{$ship_id}|{$serial_data}");
