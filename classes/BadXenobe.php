...
if (strpos ($_SERVER['PHP_SELF'], 'BadXenobe.php')) // Prevent direct access to this file
{
...
class BadXenobe
{
    public static function xenobeTrade($db)
    {
        ...
        if ($sectorinfo['port_type'] == "ore") // Port ore
        {
            ...
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Xenobe Trade Results: Sold $amount_organics Organics Sold $amount_goods Goods Bought $amount_ore Ore Cost $total_cost");
        }
        ...
        if ($sectorinfo['port_type'] == "organics") // Port organics
        {
            ...
            $trade_result2 = $db->Execute ("UPDATE {$db->prefix}universe SET port_ore = port_ore + ?, port_organics = port_organics - ?, port_goods = port_goods + ? WHERE sector_id = ?;", array ($amount_ore, $amount_organics, $amount_goods, $sectorinfo['sector_id']));
            BntDb::logDbErrors ($db, $trade_result2, __LINE__, __FILE__);
            BntPlayerLog::writeLog ($db, $playerinfo[ship_id], LOG_RAW, "Xenobe Trade Results: Sold $amount_goods Goods Sold $amount_ore Ore Bought $amount_organics Organics Cost $total_cost");
        }
        ...
        if ($sectorinfo['port_type']=="goods") // Port goods
        {
            ...
            $trade_result2 = $db->Execute ("UPDATE {$db->prefix}universe SET port_ore=port_ore + ?, port_organics = port_organics + ?, port_goods = port_goods - ? WHERE sector_id = ?;", array ($amount_ore, $amount_organics, $amount_goods, $sectorinfo['sector_id']));
            BntDb::logDbErrors ($db, $trade_result2, __LINE__, __FILE__);
            BntPlayerLog::writeLog ($db, $playerinfo[ship_id], LOG_RAW, "Xenobe Trade Results: Sold $amount_ore Ore Sold $amount_organics Organics Bought $amount_goods Goods Cost $total_cost");
        }
    }
        ...
        else  // Must have made it past planet defences
        {
            ...
            if ($shipsonplanet > 0)
            {
                while (!$resultps->EOF && $xenobeisdead < 1)
                {
                    ...
                    BadXenobe::xenobeToShip ($db, $onplanet['ship_id']);
                    $resultps->MoveNext();
                }
            }
    ...
    public static function xenobeToShip($db, $ship_id)
    {
        ...
        if (!$targetarmor > 0)
        {
            ...
            // Target had no pod
            {
                BntPlayerLog::writeLog ($db, $targetinfo['ship_id'], LOG_ATTACK_LOSE, "Xenobe $playerinfo[character_name]|N");
        ...
        if (!$attackerarmor > 0)
        {
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "$targetinfo[character_name] destroyed your ship!");
            BntPlayer::kill ($db, $playerinfo['ship_id'], false, $langvars);
            $xenobeisdead = 1;
            if ($targetarmor > 0)
            {
    ...
    public static function xenobeMove($db)
    {
        ...
        if (!$targetlink > 0) // If there is no acceptable link, use a worm hole.
        {
            ...
            while (!$targetlink > 0 && $limitloop < 15)
            {
                ...
                {
                    $targetlink = $wormto;
                    BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Used a wormhole to warp to a zone where attacks are allowed.");
                }
                $wormto++;
                $wormto++;
                $limitloop++;
        ...
        if ($targetlink > 0) // Check for sector defenses
        {
            ...
            if ($total_sector_fighters > 0 || $total_sector_mines > 0 || ($total_sector_fighters > 0 && $total_sector_mines > 0)) // If destination link has defences
            {
                ...
                {
                    BadXenobe::xenobeToSecDef ($db, $langvars); // Attack sector defences

                    return;
                }
                else
                {
                    BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Move failed, the sector is defended by $total_sector_fighters fighters and $total_sector_mines mines.");
        ...
        if ($targetlink > 0) // Move to target link
        {
            ...
            {
                $error = $db->ErrorMsg();
                BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Move failed with error: $error ");
            }
        }
        else
        {
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Move failed due to lack of target link."); // We have no target link for some reason
    ...
    public static function xenobeHunter($db)
    {
        ...
        BntDb::logDbErrors ($db, $rescount, __LINE__, __FILE__);
        ...
        if (!$targetinfo)
        {
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Hunt Failed: No Target ");
        ...
        if ($zonerow['allow_attack'] == "Y")
        {
            ...
            BntDb::logDbErrors ($db, $move_result, __LINE__, __FILE__);
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Xenobe used a wormhole to warp to sector $targetinfo[sector] where he is hunting player $targetinfo[character_name].");
            if (!$move_result)
            {
                $error = $db->ErrorMsg();
                BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Move failed with error: $error ");
            ...
            if ($total_sector_fighters > 0 || $total_sector_mines > 0 || ($total_sector_fighters > 0 && $total_sector_mines > 0)) // Destination link has defences
            {
                ...
                BadXenobe::xenobeToSecDef ($db, $langvars);
            }
            ...
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Xenobe launching an attack on $targetinfo[character_name]."); // Attack the target

            if ($targetinfo['planet_id'] > 0) // Is player target on a planet?
            {
                BadXenobe::xenobeToPlanet ($db, $targetinfo['planet_id']); // Yes, so move to that planet
            }
            else
            {
                BadXenobe::xenobeToShip ($db, $targetinfo['ship_id']); // Not on a planet, so move to the ship
            }
        }
        else
        {
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_RAW, "Xenobe hunt failed, target $targetinfo[character_name] was in a no attack zone (sector $targetinfo[sector]).");
        }
    }
