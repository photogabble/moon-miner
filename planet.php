...
$langvars = BntTranslate::load ($db, $lang, array ('bounty', 'port', 'igb', 'main', 'planet', 'report', 'common', 'global_includes', 'global_funcs', 'footer', 'news', 'combat'));
...
// If there is a planet in the sector show appropriate menu
{
    ...
    elseif ($planetinfo['owner'] == $playerinfo['ship_id'] || ($planetinfo['corp'] == $playerinfo['team'] && $playerinfo['team'] > 0))
    {
        ...
        if ($command == "sell")
        {
            ...
            else
            {
                ...
                BntDb::logDbErrors ($db, $result4b, __LINE__, __FILE__);
            }
        }
        elseif ($command == "name")
        {
        ...
        elseif ($command == "cname")
        {
            ...
            BntDb::logDbErrors ($db, $result5, __LINE__, __FILE__);
        ...
        elseif ($command == "base")
        {
            ...
            {
                BntAdminLog::writeLog ($db, 57, "{$ip}|{$playerinfo['ship_id']}|Tried to create a base without clicking on the Planet.");
            ...
            if ($planetinfo['ore'] >= $base_ore && $planetinfo['organics'] >= $base_organics && $planetinfo['goods'] >= $base_goods && $planetinfo['credits'] >= $base_credits)
            {
                ...
                else
                {
                    ...
                    BntDb::logDbErrors ($db, $update1b, __LINE__, __FILE__);
    ...
    else
    {
        ...
        elseif ($command == "attac")
        {
            ...
            {
                BntAdminLog::writeLog ($db, 57, "{$ip}|{$playerinfo['ship_id']}|Tried to start an attack without clicking on the Planet.");
                echo "You need to Click on the planet first.<br><br>";
                BntText::gotoMain ($db, $lang, $langvars);
                include './footer.php';
                die ();
        ...
        elseif ($command == "attack")
        {
            ...
            if (array_key_exists ('planet_selected', $_SESSION) == false || $_SESSION['planet_selected'] != $planet_id)
            {
                BntAdminLog::writeLog ($db, 57, "{$ip}|{$playerinfo['ship_id']}|Tried to Attack without clicking on the Planet.");
                echo "You need to Click on the planet first.<br><br>";
                BntText::gotoMain ($db, $lang, $langvars);
                include './footer.php';
                die ();
        ...
        elseif ($command == "scan")
        {
            ...
            if (array_key_exists ('planet_selected', $_SESSION) == false || $_SESSION['planet_selected'] != $planet_id)
            {
                BntAdminLog::writeLog ($db, 57, "{$ip}|{$playerinfo['ship_id']}|Tried to Scan without clicking on the Planet.");
                echo "You need to Click on the planet first.<br><br>";
                BntText::gotoMain ($db, $lang, $langvars);
                include './footer.php';
                die ();
            ...
            if ($roll > $success)
            {
                ...
                BntText::gotoMain ($db, $lang, $langvars);
                BntPlayerLog::writeLog ($db, $ownerinfo['ship_id'], LOG_PLANET_SCAN_FAIL, "$planetinfo[name]|$playerinfo[sector]|$playerinfo[character_name]");
                include './footer.php';
                die ();
            }
            else
            {
                BntPlayerLog::writeLog ($db, $ownerinfo['ship_id'], LOG_PLANET_SCAN, "$planetinfo[name]|$playerinfo[sector]|$playerinfo[character_name]");
...
//            }
        ...
        elseif ($command == "capture" &&  $planetinfo['owner'] == 0)
        {
            ...
            BntPlayerLog::writeLog ($db, $playerinfo['ship_id'], LOG_PLANET_CAPTURED, "$planetinfo[colonists]|$planetinfo[credits]|$planetowner");
        }
