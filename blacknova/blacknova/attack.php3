<?
	...
	/* check to ensure target is in the same sector as player */
        ...
        {
                echo "Target not in this sector.<BR>";
	...
	/* determine per cent chance of success in detecting target ship - based on player's sensors and opponent's cloak */
        $success=(10 - $targetinfo[cloak] + $playerinfo[sensors])*5;
        if ($success<5){$success=5;}
        if ($success>95){$sucess=95;}
	$flee=(10 - $targetinfo[engines] + $playerinfo[engines])*5;
        $roll=rand(1,100);
	$roll2=rand(1,100);
        if ($playerinfo[sector]==0) {

		/* disallow attacks in sector 0 */

		echo "You are not allowed to attack in sector 0.<BR><BR>";
		...
		if ($targetarmour<1)
		{
			...
			if ($playerarmour>0)
			{
				...
				$ship_salvage_rate=rand(0,50);
				$ship_salvage=$ship_value*$ship_salvage_rate;
