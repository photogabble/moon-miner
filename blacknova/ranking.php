...
else
{
  ...
  echo "<TR BGCOLOR=\"$color_header\"><TD><B>$l_ranks_rank</B></TD><TD><B><A HREF=ranking.php>$l_score</A></B></TD><TD><B>$l_player</B></TD><TD><B><A HREF=ranking.php?sort=turns>$l_turns_used</A></B></TD><TD><B><A HREF=ranking.php?sort=login>$l_ranks_lastlog</A></B></TD><TD><B><A HREF=ranking.php?sort=good>$l_ranks_good</A>/<A HREF=ranking.php?sort=bad>$l_ranks_evil</A></B></TD><TD><B><A HREF=ranking.php?sort=alliance>$l_team_alliance</A></B></TD><TD><B>Online</B></TD></TR>\n";
  $color = $color_line1;
  while(!$res->EOF)
  {
    $row = $res->fields;
    ...
    echo "<TR BGCOLOR=\"$color\"><TD>" . NUMBER($i) . "</TD><TD>" . NUMBER($row[score]) . "</TD><TD>$row[character_name]</TD><TD>" . NUMBER($row[turns_used]) . "</TD><TD>$row[last_login]</TD><TD>&nbsp;&nbsp;" . NUMBER($rating) . "</TD><TD>$row[team_name]&nbsp;</TD><TD>$online</TD></TR>\n";
