...
import ProfilePullOut from "@/Components/Organisms/ProfilePullOut.vue";
                ...
                <div :class="['w-full flex flex-col', {'col-span-2 pr-1': !slots.sidebar, 'px-1': slots.sidebar}]">
                    <div class="text-sm flex flex-row border-t border-ui-orange-500/50 justify-between">
                        <div class="border-t border-ui-orange-500 border-l border-partway-r p-1 px-2">
                            <span class="uppercase text-white">Turns Available:</span> {{ player.turns }}
                            <span class="text-ui-yellow">&middot;&nbsp;</span>
                            <span class="uppercase text-white">Turns Used:</span> {{ player.turns_used }}
                            <span class="text-ui-yellow">&middot;&nbsp;</span>
                            <span class="uppercase text-white">Credits</span> {{ player.credits }}
                        </div>

                        <div class="border-ui-orange-500 border-partway-t px-2 p-1">
                            <span class="uppercase text-white">Score</span> {{ player.score }}
                        </div>
                    </div>
                    <slot/>
                </div>

                <navigation-column is-logged-in />
            </div>
        </main>
    </div>
</template>
